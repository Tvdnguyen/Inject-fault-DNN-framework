...
def quant_feature_old(x, min_val, max_val, num_faults):
    # Add quantization after activation function
    x = quantize_tensor(x, min_val=min_val, max_val=max_val)
    x_inj = inject_tensor_faults(x, num_faults)
    scale, zero_point = compute_scale_zero_point(x_inj.min(), x_inj.max(), 16)
    x = QTensor(tensor=x_inj, scale=scale, zero_point=zero_point)
    x = dequantize_tensor(x)
    return x
...
def calculate_total_faults(fault_rate):
    ...
    total_bits_weight = total_weights * 8  # Mỗi trọng số là 8 bits
    total_bits_act = 6508 * 8
...
def distribute_faults_randomly(total_faults, num_parts=8):
...
def forward_quantize_fix(model, x, stats, fault_rate):
    ...
    faults_per_layer = distribute_faults_randomly(total_faults, num_parts=8)
    ...
    x = quant_feature(x, min_val=stats["conv1"]["min"], max_val=stats["conv1"]["max"], num_faults= faults_per_layer[1])
    # x = dequantize_tensor(QTensor(tensor=x, scale=scale_next, zero_point=zero_point_next))
    x = F.max_pool2d(x, 2, 2)
    x, scale_next, zero_point_next = quantize_layer(x, model.conv2, stats["fc"], scale_next, zero_point_next, faults_per_layer[2])
    x = quant_feature(x, min_val=stats["conv2"]["min"], max_val=stats["conv2"]["max"], num_faults= faults_per_layer[3])
    # x = dequantize_tensor(QTensor(tensor=x, scale=scale_next, zero_point=zero_point_next))
    x = F.max_pool2d(x, 2, 2)
    x = x.view(-1, 5*5*16)
    x, scale_next, zero_point_next = quantize_layer(x, model.fc, stats["fc1"], scale_next, zero_point_next, faults_per_layer[4])
    x = quant_feature(x, min_val=stats["fc"]["min"], max_val=stats["fc"]["max"], num_faults= faults_per_layer[5])
    # x = dequantize_tensor(QTensor(tensor=x, scale=scale_next, zero_point=zero_point_next))
    x, scale_next, zero_point_next = quantize_layer(x, model.fc1, stats["fc2"], scale_next, zero_point_next, faults_per_layer[6])
    x = quant_feature(x, min_val=stats["fc1"]["min"], max_val=stats["fc1"]["max"], num_faults= faults_per_layer[7])
    # x = dequantize_tensor(QTensor(tensor=x, scale=scale_next, zero_point=zero_point_next))
    x = model.fc2(x)
...
file_path = "result/result_framework_accuracy_8bit_fixpoint.txt"
